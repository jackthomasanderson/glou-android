...
Write-Host "?? ?tape 1/5: V?rification de Flutter..." -ForegroundColor Yellow
$flutterExists = Get-Command flutter -ErrorAction SilentlyContinue

if ($flutterExists) {
    Write-Host "? Flutter est d?j? install?!" -ForegroundColor Green
    flutter --version
} else {
    Write-Host "??  Flutter n'est pas install?." -ForegroundColor Red
    Write-Host ""
    ...
    Write-Host "? Flutter install?!" -ForegroundColor Green
}

Write-Host ""
Write-Host "?? ?tape 2/5: V?rification de l'environnement Flutter..." -ForegroundColor Yellow
flutter doctor -v

Write-Host ""
Write-Host "?? ?tape 3/5: Installation des d?pendances..." -ForegroundColor Yellow
flutter pub get
...
Write-Host ""
Write-Host "?? ?tape 4/5: Nettoyage du projet..." -ForegroundColor Yellow
flutter clean

Write-Host ""
Write-Host "?? ?tape 5/5: Compilation de l'APK..." -ForegroundColor Yellow
flutter build apk --release

if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "? ?? L'application a ?t? compil?e avec succ?s!" -ForegroundColor Green
    Write-Host ""
    Write-Host "?? Fichier APK cr??:" -ForegroundColor Cyan
